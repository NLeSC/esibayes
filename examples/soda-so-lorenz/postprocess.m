...
% from the soda-mode optimization results:
load('./results/soda-so-results.mat','conf','evalResults',...
...
[x,y] = mmsodaCalcUncertInts(conf,evalResults,metropolisRejects,prc,evalNumbers,'iStateKF',iStateKF);
...
hold on
h2 = plot(x,y(2,:),'-k');
h3 = plot(conf.priorTimes,constants.obsState(iStateKF,:),'sm');
set(h1,'facecolor',0.5*[1,1,1],'edgecolor',0.5*[1,1,1])
set(h3,'markerfacecolor','m','markersize',4)
xlabel('Time')
ylabel('State')
box on
