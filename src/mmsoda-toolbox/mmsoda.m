...
if ~isempty(varargin) && ischar(varargin{1})
    ...
    if conf.parameterSamplesAreGiven
        ...
        try
            ...
            error(['When ''parameterSamplesAreGiven'' is true, you should provide the ',...
                   'file ''./data/parameter-samples.mat'' with the parameter samples ',...
                   'as an array ''samples''.'])
        end
        
        if size(samples,2)~=conf.nOptPars
            error(['The number of parameters in the provided ''samples'' array is not equal ',...
                'to the number of parameters in the configuration.'])
        end
