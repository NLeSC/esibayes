...
\smallq{Go to \burl{https://github.com/NLeSC/esibayes} and click the `Download ZIP' button to download the tutorial files needed when working through this chapter, as well as the files pertaining to the MMSODA Toolbox for MATLAB, which we will use later. Unzip it into your (local) home directory or a USB drive.}

\smallq{In WinSCP, use the left pane to navigate to the directory where you unzipped the files. Change directory into the `manual' subdirectory. Copy the `tutorial' directory to the remote directory by selecting it on the left and pressing the F5 button. The tutorial directory should now be present in the right pane as well.}
...
\end{lstlisting}\index{Linux commands!ssh@\texttt{ssh}}
...
\end{lstlisting}\index{Linux commands!rm@\texttt{rm}}
...
You can check the status of the job by typing the \lstinline[style=bashinline]{qstat} command, followed by the job id, like so:\index{Linux commands!qstat@\texttt{qstat}}
\begin{lstlisting}[style=basic,style=bash]
...
jspaaks@login2:~/tutorial/simple-batch$
\end{lstlisting}
You can look up statistics for a given node at  \burl{https://ganglia.surfsara.nl/?r=hour&cs=&ce=&s=by+name&c=LISA%2520Cluster&tab=m&vn=}\index{Ganglia}, for example if you want to check its performance (see also Fig.~\ref{fig:ganglia-screenshot}).
...
\item{the serial queue}\index{queue!serial}
\item{the parallel queue}\index{queue!parallel}
\end{enumerate}

\begin{figure}[hbtp]
  \centering
    \includegraphics[width=\textwidth]{./../eps/ganglia-screenshot.eps}
...
If you want to check how much computing time your \textbf{acc}ount has \textbf{use}d, you can use the \texttt{accuse}\index{Linux commands!accuse@\texttt{accuse}} command, or the \textbf{acc}ount \textbf{info}rmation (\texttt{accinfo}\index{Linux commands!accinfo@\texttt{accinfo}}) command.
